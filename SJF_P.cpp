...
struct cmp_readyQ
{
    ...
    {
        long long int remTime1 = (p1.isBT1Complete == false) ? p1.Burst_time_1 : p1.Burst_time_2;
        long long int remTime2 = (p2.isBT1Complete == false) ? p2.Burst_time_1 : p2.Burst_time_2;

        if(remTime1 == remTime2)
            return p1.PID > p2.PID;

       return (remTime1 > remTime2 );
    }
};
...
public:
    ...
    priority_queue<Process, vector<Process>, cmp_readyQ> ready_queue;  //min_heap of rem CPU burst times
    ...
    void run(Process& curr_proc){                                        //
        ...
        while(remTime > 0){
            ...
            each_clock_tick();
            // cout << "Debugging : remTime : " << remTime << "\n";

            //if another process with less BT arrives in ready queue, what to do
            if(ready_queue.size() ){
                long long int remTime_temp = (ready_queue.top().isBT1Complete == false) ? ready_queue.top().Burst_time_1_dup : ready_queue.top().Burst_time_2_dup;
                if(remTime_temp < remTime){
    ...
    void schedule(){
        ...
        cout<<"Initialised ready queue\n";
        //check if ready queue has any process, if yes, schedule it based on SJF Pre-emptive
        while(ready_queue.size() || new_queue.size()){
            ...
            run(curr_proc);
            cout<<"Done executing process " << curr_proc.PID << "\n";
        }
...
int main(){
    //freopen("output_sjf_p.txt", "w", stdout);
    ...
    //create SJF Pre-emptive scheduler object and pass list of all processes to it for constructor 
    CPUScheduler sjf_p_scheduler(processes);
    
    //invoke SJF Pre-emptive scheduler
    sjf_p_scheduler.schedule();

    //print gant chart
    // for(auto s : sjf_p_scheduler.gant_chart){
    //     cout << s << "\n";
    // }
    // cout << "\n";
    ...
    for(auto p : sjf_p_scheduler.terminated_q){
    ...
    vector<Process> completedProcessesVector = sjf_p_scheduler.terminated_q;
    ...
    //store values in a file without using streams
    FILE *fp;
    fp = fopen("all.csv", "a+");
    ...
    {
        for(int i=0;i<completedProcessesVector.size();i++){
            fprintf (pFile, "%f,%f,%f,%f,%lld\n",completionTime[i],TAT[i],waitingTime[i],responseTime[i],completedProcessesVector[i].PID);
        }
        fclose (pFile);
    }
    system("python3 graph.py SJFP.jpeg SJFP");
	system("python3 graph1.py SJFPPie.jpeg SJFP");
    system("python3 stack.py SJFPstack.jpeg SJFP");
    remove("out.csv");
